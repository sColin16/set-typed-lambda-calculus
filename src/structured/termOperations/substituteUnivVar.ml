...
and substitute_univ_var_type_rec (variable_num : int)
    ...
    =
  (* If the universal type variable doesn't exist in the type, do nothing to prevent unnecessarily
     adding to the recursive type context *)
  if not (target_substitution_exists variable_num in_type) then in_type
  else
    ...
    (* Perform the substitutions in the union type *)
    let substituted_union =
      substitute_univ_var_union variable_num with_type
        recontextualized_type.union
    in
    (* Determine the substitutions that need to happen in the context*)
    let context_substitutions =
      get_context_substitutions
        { variable_num; with_type }
        recontextualized_type.union recontextualized_type.context
    in
    (* Perform the substitutions in the context *)
    let substituted_contexts =
      substitute_univ_var_context recontextualized_type.context
        context_substitutions
    in
    let substituted_type =
      build_structured_type substituted_union substituted_contexts
    in
    substituted_type

and substitute_univ_var_context (context : recursive_context)
    (context_subs : context_subs) =
  List.mapi
    (fun idx context_def ->
      substitute_univ_var_context_def context_def
        (IntMap.find_opt idx context_subs))
    context

and substitute_univ_var_context_def ({ kind; flat_union } : recursive_def)
    (sub_directive_opt : substitute_directive option) =
  if Option.is_none sub_directive_opt then { kind; flat_union }
  else
    let { variable_num; with_type } = Option.get sub_directive_opt in
    {
      kind;
      flat_union =
        substitute_univ_var_flat_union variable_num with_type flat_union;
    }

and substitute_univ_var_flat_union (variable_num : int)
    (with_type : structured_type) (flat_union : flat_union_type) =
  ...
  let substituted_union =
    substitute_univ_var_union variable_num with_type unflattened_union
  in
  (* Convert to a flat union, using the with_type context where recursive variables may be from*)
  let substituted_flat_union =
    flatten_union substituted_union with_type.context
  in
  substituted_flat_union

and substitute_univ_var_function (variable_num : int)
    (with_type : structured_type) ((arg, return) : unary_function) =
  map_pair (substitute_univ_var_union variable_num with_type) (arg, return)

and substitute_univ_var_union (variable_num : int) (with_type : structured_type)
    (union : union_type) =
  List.flatten
    (List.map (substitute_univ_var_base variable_num with_type) union)

and substitute_univ_var_base (variable_num : int) (with_type : structured_type)
    (base : base_type) =
  match base with
  ...
  | Intersection branches ->
      let substituted_branches =
        List.map (substitute_univ_var_function variable_num with_type) branches
      in
  ...
  | UnivQuantification inner_type ->
      let new_var_num = variable_num + 1 in
      let new_with_type = shift_univ_var_type with_type 1 in
      let substituted_inner_type =
        substitute_univ_var_union new_var_num new_with_type inner_type
      in
      [ UnivQuantification substituted_inner_type ]
  (* Type variables are substituted when they match the target variable number *)
  | UnivTypeVar num ->
      if num = variable_num then with_type.union else [ UnivTypeVar num ]

and target_substitution_exists (var_num : int) (in_type : structured_type) =
  target_substitution_exists_context var_num in_type.context
  || target_substitution_exists_union var_num in_type.union

and target_substitution_exists_context (var_num : int)
    (context : recursive_context) =
  List.exists (target_substitution_exists_rec_def var_num) context

and target_substitution_exists_union (var_num : int) (union : union_type) =
  List.exists (target_substitution_exists_base var_num) union

and target_substitution_exists_rec_def (var_num : int) (rec_def : recursive_def)
    =
  target_substitution_exists_flat_union var_num rec_def.flat_union

and target_substitution_exists_base (var_num : int) (base : base_type) =
  match base with
  (* Labels and recursive type variables can't contain the substitution (recursive context is checked separately) *)
  | Label _ | RecTypeVar _ -> false
  (* Intersection is recursive across all branches *)
  | Intersection branches ->
      List.exists (target_substitution_exists_function var_num) branches
  (* We add one ot the variable number when we pass through universal quantification since binding quantifier is one further away *)
  | UnivQuantification inner_union ->
      target_substitution_exists_union (var_num + 1) inner_union
  | UnivTypeVar inner_num -> var_num = inner_num

and target_substitution_exists_flat_union (var_num : int)
    (flat_union : flat_union_type) =
  let unflattened_union = unflatten_union flat_union in
  target_substitution_exists_union var_num unflattened_union

and target_substitution_exists_function (var_num : int)
    ((arg, return) : unary_function) =
  target_substitution_exists_union var_num arg
  || target_substitution_exists_union var_num return
