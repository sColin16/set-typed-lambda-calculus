...
and is_well_founded_union_rec (union : union_type) (context : recursive_context)
    (visited : TypeVarSet.t) =
...
and is_well_founded_base (context : recursive_context) (visited : TypeVarSet.t)
    (base : base_type) =
  match base with
  ...
  | Intersection functions ->
      List.for_all (is_well_founded_func context visited) functions
  | RecTypeVar n ->
      (* If we encounter a type-var loop, this branch of the type is not well-founded *)
      if TypeVarSet.mem n visited then false
      else
...
and is_well_founded_func (context : recursive_context) (visited : TypeVarSet.t)
    ((arg, return) : unary_function) =
  (* A function type is well-founded if bot the arguments and return type are *)
  is_well_founded_union_rec arg context visited
  && is_well_founded_union_rec return context visited
