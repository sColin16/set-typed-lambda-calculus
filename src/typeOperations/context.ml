...
let get_type_in_context (t : recursive_type)
    (recursive_context : recursive_context) : recursive_type =
...
(* Converts a list of recursive types into a list of union types and a common context they all share,
   shifting recursive type variables in the union as appropriate *)
let get_unified_type_context (types : recursive_type list) =
